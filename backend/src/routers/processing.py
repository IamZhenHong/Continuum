...
async def process_resource(resource_id: int, user_id: int, message: str):
    try:
        logging.info(f"🚀 Starting processing for Resource ID: {resource_id} (User ID: {user_id})")
        ...
        user_status = redis_client.get(f"{settings.REDIS.REDIS_RESOURCE_STATUS_PREFIX}{user_id}")
        logging.info(f"🔄 Redis Queue Before Processing: {queue_status}, User {user_id} Status: {user_status}")

        # ✅ Simulate AI Processing (Replace with real logic)
        logging.info(f"🧠 Simulating AI processing for {settings.PROCESSING_TIME_ESTIMATE} seconds...")

        print(resource_id)
        print(user_id)
        print(message)
        await enrich(schemas.EnrichResourceRequest(resource_id=resource_id, message=message,user_id=user_id))
        ...
        queue_status_after = redis_client.get(settings.REDIS.REDIS_QUEUE_PROCESSING_COUNT)
        user_status_after = redis_client.get(f"{settings.REDIS.REDIS_RESOURCE_STATUS_PREFIX}{user_id}")
        logging.info(f"🛠️ Redis Queue After Processing: {queue_status_after}, User {user_id} Status: {user_status_after}")

        # ✅ Update status in Supabase
        logging.info(f"📝 Updating Supabase status for Resource ID: {resource_id}")
        update_response = supabase_client.table("processing_queue").update({
            "status": "completed",
            "completed_at": "now()"
        ...
        if update_response.data:
            ...
            logging.error(f"❌ Supabase update failed for Resource ID: {resource_id}. Response: {update_response}")
...
async def start_processing(background_tasks: BackgroundTasks):
    ...
    for job in jobs.data:
        ...
        task = asyncio.create_task(process_resource(resource_id, user_id,message.data[0]["message"]))
...
async def add_to_processing_queue(data: schemas.AddToProcessingQueueRequest):
    ...
    insert_response = supabase_client.table("processing_queue").insert({
    ...
    if insert_response.data:
        ...
        logging.error(f"❌ Error adding Resource ID {data.resource_id} to processing queue")
